...
def plot_sentiment(
    ...
    try:
        ...
        df = pd.read_csv(
            ...
            low_memory=False,
            names=[
                "parse_count", "author_id", "restaurant_name", "rating_review",
                "sentiment", "review_id", "title_review", "review_preview",
                "review_full", "date", "city", "url_restaurant",
            ],
        )

        # Convert dates and ratings to proper formats
        df["date"] = pd.to_datetime(df["date"], format="%d-%b-%y", errors="coerce")
        df = df.dropna(subset=["date", "rating_review"])
        df["rating_review"] = pd.to_numeric(df["rating_review"], errors="coerce").astype("Int64")
